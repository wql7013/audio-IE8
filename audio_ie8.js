 ...
 */
(function (b) {
    "function" == typeof define && define.amd ? define(['require', 'jquery'], b) : window.audioIE8 = b.apply(jQuery('head > script').last().attr('src'), jQuery, jQuery.jPlayer);
})(function (require, $) {
    var oReturn = {};/*模块返回值*/
    var aReadyCallBack = [];
                    ...
                    if (this.readyState > 0 ) {
                        ...
                        $(this).on('loadedmetadata', function () {
                            ...
                            if (this.error.code == MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED || this.error.code == MediaError.MEDIA_ERR_DECODE) {
                                ...
                                ErrorDialog(this.error.code, MediaError,$(this).attr('src'));
                                nReadyNum++;
                                if (nReadyNum == nAudioNum) {
                                    fReady();
                                }
...
//            初始化jplayer播放器, 绑定audio DOM事件
            var $thisPlayer = $('<div>', {
                ...
                error: function (event) {/*发生错误时忽略,防止影响页面其他展示*/
                    ErrorDialog(event.jPlayer.error.type,$.jPlayer.error,$thisAudio.attr('src'));
...
//            audio DOM添加属性和方法
    ...
    function ErrorDialog(errCode,errType,audioPath) {
